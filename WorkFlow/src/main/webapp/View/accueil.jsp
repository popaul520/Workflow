...
<body>
	...
	<div class="main-container">
			...
			<div class="user-controls">
				<form action="search" method="get">
					<input type="text" name="q" class="search-bar"
						placeholder="ID ou Nom du dossier..."
						value="<%=request.getParameter("q") != null ? request.getParameter("q") : ""%>">
				</form>
				<%
				if (session.getAttribute("user") != null) {
				%>
				<a href="profil" class="profile-btn">Mon Profil</a> <a href="logout"
					class="logout-link">Déconnexion</a>
				<%
				} else {
				%>
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td><%=wf.getTitre()%></td>
						<td><span style="color: #666; font-size: 0.9em;"> <%=(wf.getDateCreation() != null) ? sdf.format(wf.getDateCreation()) : "N/C"%>
						</span></td>
						<td><span style="color: #888; font-size: 0.9em;">Admin</span></td>
						<td><span
							style="font-size: 0.85em; background: <%=badgeBg%>; color: <%=badgeText%>; padding: 4px 12px; border-radius: 12px; font-weight: bold; display: inline-block; min-width: 100px; text-align: center; border: 1px solid rgba(0,0,0,0.05);">
								<%=libelleEtape%>
						</span></td>
						<td><a href="saisie-etape?id_workflow=<%=wf.getId()%>"
							class="btn-view">Voir</a></td>
					</tr>
					<%
					}
					} else {
					...
					<tr>
						<td colspan="6"
							style="text-align: center; padding: 30px; color: #999;">Aucun
							workflow en cours.</td>
					</tr>
					<%
					}
					%>
						...
						<c:when test="${not empty pendingList}">
							<c:forEach var="wfAction" items="${pendingList}">
								<tr>
									...
									<td><span class="badge-urgent"
										style="background: #fed7d7; color: #822727; padding: 5px 10px; border-radius: 15px; font-weight: bold; font-size: 0.8em;">
											ÉTAPE SUIVANTE </span></td>
									<td><a href="saisie-etape?id_workflow=${wfAction.id}" class="btn-view"
						...
						<c:otherwise>
							<tr>
								<td colspan="4"
									...
									Aucune action requise pour votre rôle
									(${sessionScope.user.role}).</td>
							</tr>
						</c:otherwise>
					</c:choose>
				</tbody>
		...
		<div class="sidebar">
			<h3>Workflow</h3>
			<ul>
				<li class="${currentStatus == 'tous' ? 'active' : ''}"><a
					href="home?status=tous">🏠 Tous les dossiers</a></li>
				<li class="${currentStatus == 'en_cours' ? 'active' : ''}"><a
					href="home?status=en_cours">⏳ En cours</a></li>
				<li class="${currentStatus == 'termine' ? 'active' : ''}"><a
					href="home?status=termine">✅ Terminé</a></li>
				<li class="${currentStatus == 'annule' ? 'active' : ''}"><a
					href="home?status=annule">❌ Annulé </a></li>
