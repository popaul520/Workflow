...
<head>
    ...
    <style>
        ...
        .step-head { color: #ccc; font-weight: bold; font-size: 0.9em; transition: 0.3s; padding-bottom: 5px; }
        .step-head.active { color: #3498db; border-bottom: 2px solid #3498db; }
        ...
        input, select, textarea { padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px; background-color: #fafafa; }
        input:focus, select:focus, textarea:focus { border-color: #3498db; outline: none; background-color: #fff; }
...
<body>
        ...
        <div class="form-card">
            ...
            <form action="creer-workflow" method="post" id="creationForm">
                ...
                <div class="form-step" id="step-2">
                    ...
                    <div class="grid-form" id="conteneur-dynamique">
                        </div>
    ...
    <script>
        ...
        function chargerChampsDynamiques(idTemplate) {
            ...
            // Récupération de la structure via l'API Servlet
            fetch('creer-workflow?action=getChampsJson&id_template=' + idTemplate)
                ...
                .catch(err => {
                    ...
                    alert("Erreur lors de la récupération des spécifications du template.");
                });
        }
        ...
        function construireFormulaireDynamique() {
            ...
            if(structureChamps.length === 0) {
                conteneur.innerHTML = "<p class='full' style='color:#718096; font-style:italic;'>Aucune donnée spécifique demandée à l'étape 1 pour ce modèle.</p>";
                return;
            }

            // Injection masquée du nombre total de champs à traiter par le contrôleur
            conteneur.innerHTML += `<input type="hidden" name="total_champs" value="\${structureChamps.length}">`;
            ...
            structureChamps.forEach((champ, index) => {
                ...
                // Gestion des composants de saisie selon les contraintes
                if (champ.refContrainte === 'Bool') {
                    ...
                    let inputType = champ.typeComposant ? champ.typeComposant : 'text';
                    htmlChamp += `<input type="\${inputType}" name="attr_\${index}" \${champ.estObligatoire ? 'required' : ''} placeholder="Saisir...">`;
                }

                // Gestion des commentaires et des dates optionnelles annexées au champ
                if (champ.aCommentaire) {
                    htmlChamp += `<input type="text" name="comm_\${index}" style="margin-top:5px;" placeholder="Remarque optionnelle...">`;
                }
                if (champ.aDate) {
                    htmlChamp += `<input type="date" name="date_\${index}" style="margin-top:5px;">`;
                }

                htmlChamp += `</div>`;
                conteneur.innerHTML += htmlChamp;
            });
        }

        function versEtapeSaisie() {
            if(!document.getElementById('wf-titre').value.trim()) {
                alert("Veuillez saisir un libellé de projet.");
                return;
            }
